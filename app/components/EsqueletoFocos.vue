  ...
  <div>
    ...
    <svg viewBox="0 0 440 700" class="w-full" role="listbox"
      ...
      <g filter="url(#pkBoneShadow)">
        <path d="M192,44 Q192,10 220,10 Q248,10 248,44 Q248,66 233,72 Q220,77 207,72 Q192,66 192,44 Z" fill="url(#pkBone)" stroke="#7d8593" stroke-width="0.6" stroke-opacity="0.7" />
        <path d="M192,44 Q192,10 220,10 Q248,10 248,44 Q248,66 233,72 Q220,77 207,72 Q192,66 192,44 Z" fill="url(#pkBoneHi)" />
        <path d="M210,66 Q220,75 230,66 L228,76 Q220,82 212,76 Z" fill="url(#pkBone)" stroke="#7d8593" stroke-width="0.6" stroke-opacity="0.7" />
        ...
        <g v-for="(v, i) in SK_VERTEBRAS" :key="'v' + i">
          <rect :x="v.x" :y="v.y" :width="v.w" :height="v.h" :rx="Math.min(v.h / 2, 7)" fill="url(#pkBoneV)" stroke="#7d8593" stroke-width="0.6" stroke-opacity="0.7" />
          <rect :x="v.x + 1.5" :y="v.y + 1" :width="v.w - 3" :height="v.h * 0.4" :rx="Math.min(v.h / 2, 7) * 0.7" fill="#fffdf8" opacity="0.35" />
        </g>
        <path d="M202,485 Q220,483 238,485 L231,538 Q220,547 209,538 Z" fill="url(#pkBoneV)" stroke="#7d8593" stroke-width="0.6" stroke-opacity="0.7" />
        <path d="M201,487 C152,485 122,520 130,560 C135,588 168,596 187,574 C200,558 203,520 201,487 Z" fill="url(#pkBone)" stroke="#7d8593" stroke-width="0.6" stroke-opacity="0.7" />
        <path d="M239,487 C288,485 318,520 310,560 C305,588 272,596 253,574 C240,558 237,520 239,487 Z" fill="url(#pkBone)" stroke="#7d8593" stroke-width="0.6" stroke-opacity="0.7" />
        <path d="M201,487 C152,485 122,520 130,560 C135,588 168,596 187,574 C200,558 203,520 201,487 Z" fill="url(#pkBoneHi)" />
        <circle cx="151" cy="600" r="12.5" fill="url(#pkBone)" stroke="#7d8593" stroke-width="0.6" stroke-opacity="0.7" />
        <rect x="150" y="606" width="13" height="92" rx="6.5" fill="url(#pkBoneV)" stroke="#7d8593" stroke-width="0.6" stroke-opacity="0.7" />
        <circle cx="289" cy="600" r="12.5" fill="url(#pkBone)" stroke="#7d8593" stroke-width="0.6" stroke-opacity="0.7" />
        <rect x="277" y="606" width="13" height="92" rx="6.5" fill="url(#pkBoneV)" stroke="#7d8593" stroke-width="0.6" stroke-opacity="0.7" />
        <path d="M114,182 Q146,188 146,192 L141,242 Q138,246 134,240 Z" fill="url(#pkBone)" stroke="#7d8593" stroke-width="0.6" stroke-opacity="0.7" />
        <path d="M326,182 Q294,188 294,192 L299,242 Q302,246 306,240 Z" fill="url(#pkBone)" stroke="#7d8593" stroke-width="0.6" stroke-opacity="0.7" />
      </g>
      <g font-family="JetBrains Mono, monospace" font-size="9" fill="#aeb6c2" font-weight="600">
        ...
        <line v-for="tk in SK_TICKS" :key="'l' + tk.t" x1="346" :y1="tk.y" x2="354" :y2="tk.y" stroke="#7d8593" stroke-width="1" />
      </g>
...
/* Diana táctil: SK_HIT vale 14 sobre un viewBox de 440, que en el contenedor real son ~21 px de
   diámetro, por debajo del mínimo de 44. No se sube en `#mapa-focos` porque ese valor lo usa
   también /mapa-metastasis, que nadie ha pedido tocar: se sube SOLO aquí. */
const HIT = Math.max(SK_HIT, 22)
...
<style scoped>
/* Foco de teclado propio: sobre el panel oscuro nuevo, el outline automático del navegador
   (azul de sistema) queda pobre. Blanco cálido, el mismo de los rótulos de los otros visores. */

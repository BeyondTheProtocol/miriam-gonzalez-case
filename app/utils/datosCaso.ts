...
export type Forma = 'circulo' | 'cuadrado' | 'triangulo' | 'rombo' | 'aspa' | 'estrella' | 'banderin'
...
const _pathForma = (forma: Forma, x: number, y: number, r: number): string => {
  switch (forma) {
    ...
    case 'banderin': // progresión: raya con banderín (rima con la raya de progresión de los gráficos).
      // ▲▼ quedan para «fuera de rango»; el octágono se probó y a 9-11 px se leía como un círculo.
      return `M${x - 0.8},${y - r * 1.4}h1.6v${r * 2.8}h-1.6Z` +
        `M${x + 0.8},${y - r * 1.4}L${x + r * 1.6},${y - r * 0.85}L${x + 0.8},${y - r * 0.3}Z`
    case 'estrella': {
      let d = ''
      for (let i = 0; i < 10; i++) {

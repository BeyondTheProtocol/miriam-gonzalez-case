<template>
  <div>
    <SectionHero />
    <section
      class="border-b border-gold-300/50 bg-gradient-to-r from-gold-100 via-ink-50 to-ocean-50"
    >
    ...
    <section
      ...
      :aria-label="$t('index.molecular_profile')"
    >
    ...
    <section
      class="section-spacing bg-white/50"
      :aria-label="$t('index.in_the_press')"
    >
      <div class="section-container">
        <h2
          class="heading-display text-2xl sm:text-3xl text-ink-950 mb-10 text-center"
        >
          {{ $t('index.in_the_press') }}
        </h2>
        <div
          class="flex flex-wrap justify-center gap-8 sm:gap-12"
          v-if="pressData?.articles"
        >
          <a
          ...
          >
            <span
              class="font-display font-bold text-lg text-ink-600 group-hover:text-ink-900 transition-colors"
            >
              {{ article.outlet }}
            </span>
    ...
    <section
      class="py-20 bg-ink-950 text-white relative overflow-hidden"
      :aria-label="$t('index.core_thesis')"
    >
      <div
      ...
      <div class="relative section-container text-center">
        <span
          class="tag bg-gold-500/20 text-gold-300 border-gold-500/30 mb-6 inline-flex"
        >
          {{ $t('index.core_thesis') }}
        </span>
        <blockquote
          class="font-display text-2xl sm:text-3xl lg:text-4xl font-semibold leading-snug max-w-3xl mx-auto"
        >
          {{ $t('ciencia.thesis') }}
        </blockquote>
...
defineOgImage('Default.takumi', {
  title:
    locale.value === 'es'
      ? 'Carcinoma NE IV — perfil multirresistente'
      : 'Stage IV NE breast — multi-resistant profile',
  description:
    locale.value === 'es'
      ? 'RB1/ESR1 en plasma · FGFR1/CCND1 confirmados · urgen terapias no luminales.'
      : 'Plasma RB1/ESR1 · confirmed FGFR1/CCND1 · non-luminal targeted therapy is urgent.',
  colorMode: 'light',
})

    ...
    <section class="section-spacing" :aria-label="$t('science.title')">
      <div class="section-container">
        ...
        <div class="space-y-4 mb-14">
          <div
            v-for="tx in treatments"
            :key="tx.line"
            class="card-base flex items-start gap-4"
          >
            <span
            ...
            <div>
              <h4 class="font-semibold text-ink-900 text-sm">
                {{ tx.regimen }}
              </h4>
        ...
        <div class="space-y-8 mb-14">
          ...
          >
            <div v-if="section.title" class="mb-4">
              <h3
                class="font-display font-semibold text-ink-900 text-lg mb-1.5"
              >
                {{ section.title }}
              </h3>
              <p
                v-if="section.subtitle"
                class="text-sm text-ink-700 leading-relaxed"
              >
                {{ section.subtitle }}
              </p>
            </div>
              ...
              >
                <div class="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <h4 class="font-semibold text-ink-900 text-sm mb-1.5">
                      {{ paper.finding }}
                    </h4>
        ...
        <section v-if="false">
          <h2
            id="panel-title"
            class="heading-display text-2xl text-ink-950 mb-6"
          >
              ...
              >
                ...
                <thead>
                  <tr class="border-b-2 border-ink-200">
                    <th
                      scope="col"
                      class="text-left py-2 pr-3 font-semibold text-ink-800 text-xs whitespace-nowrap"
                    >
                      {{ $t('ciencia.component') }}
                    </th>
                    <th
                    ...
                    <th
                      scope="col"
                      class="text-left py-2 pr-3 font-semibold text-ink-800 text-xs"
                    >
                      {{ $t('ciencia.targets') }}
                    </th>
                    <th
                      scope="col"
                      class="text-left py-2 font-semibold text-ink-800 text-xs"
                    >
                ...
                <tbody>
                  ...
                  >
                    <td
                      class="py-2.5 pr-3 text-xs font-medium text-ink-900 whitespace-nowrap"
                    >
                      {{ row.component }}
                    </td>
                    <td
                      class="py-2.5 pr-3 text-xs text-ink-600 whitespace-nowrap"
                    >
                      {{ row.method }}
                    </td>
                    <td class="py-2.5 pr-3 text-xs text-ink-600">
                      {{ row.targets }}
                    </td>
                    <td class="py-2.5 text-xs text-ink-600">
                      {{ row.implication }}
                    </td>
                  </tr>
                </tbody>
        ...
        <div class="card-base bg-ink-950 text-white border-0 mb-16">
          <div class="flex items-center gap-2.5 mb-4">
            <span
              class="w-9 h-9 rounded-xl bg-gold-500/20 flex items-center justify-center"
            >
        ...
        <div v-if="articles && articles.length > 0">
          ...
          <div class="space-y-4">
            ...
            >
              <div class="flex-1 min-w-0">
                <div class="flex flex-wrap items-center gap-1.5 mb-2">
                  <span
                    ...
                    :key="tag"
                    class="tag-ocean text-xs"
                    >{{ tag }}</span
                  >
                </div>
                <h3
                  class="font-semibold text-ink-900 text-sm mb-1 group-hover:text-ocean-700 transition-colors"
                >
                  {{ article.title }}
                </h3>
              ...
              <Icon
                name="ph:arrow-right"
                class="shrink-0 w-4 h-4 text-ink-400 group-hover:text-ocean-600 transition-colors mt-0.5"
                aria-hidden="true"
              />
            </NuxtLink>
          </div>

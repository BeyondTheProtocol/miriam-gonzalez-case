    ...
    <section class="section-spacing" aria-label="Mapa de metástasis">
      ...
      <div class="px-4 sm:px-6 lg:px-8 max-w-[1280px] mx-auto">
        ...
        <!-- Aviso PERSISTENTE · herramienta de APOYO (petición de la paciente:
             ...
             visible con el callout canónico del DS (.alert-callout). El detalle
             de método (estudios, SUV, reconstrucción) queda plegado debajo para
             no empujar lo primario, pero el encuadre «apoyo, no diagnóstico» no
             se esconde nunca. role=note → el lector de pantalla lo anuncia. -->
        ...
        <section class="order-1 mb-14" aria-labelledby="dianas-resumen">
          <div class="flex items-baseline justify-between flex-wrap gap-x-3 gap-y-2 mb-2">
            ...
            <span class="status-badge self-start mt-1" :style="{ background: '#e8d4ed', color: '#5a2466' }">{{ L('equipa, no indica', 'equips, does not indicate') }}</span>
          </div>
            ...
            <div v-if="isMultiFocusBone" class="mb-3 rounded-card border border-[rgba(45,27,61,0.12)] bg-cream-card p-3">
              ...
              <div>
                ...
                <div v-else class="rounded-card border px-3 py-2 text-[12.5px] leading-snug flex items-start gap-2"
                  ...
                  <span v-if="sel.fdg != null" class="status-badge shrink-0" style="background:#fde4cc;color:#8a4a1a">{{ isAiDavid(sel) ? L('IA · por confirmar', 'AI · to confirm') : L('NUEVO', 'NEW') }}</span>
...
.zone-divider {
  ...
  border-top: 3px solid #9d44ab;
  background:
    linear-gradient(180deg, rgba(157, 68, 171, 0.06), rgba(157, 68, 171, 0) 70%),
    #f5efe6;
  text-align: center;
}
.zone-divider__eyebrow {
...
@media (prefers-reduced-motion: reduce) {
  .zone-divider { scroll-behavior: auto; }
}
